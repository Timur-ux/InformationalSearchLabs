...
DataBase::DataBase() {
  ...
  if (!storagePath) {
    LOG_WARNING() << "environment variable [STORAGE PATH] not set, so i use random generated directory in /tmp folder";
...
void DataBase::insert(id_t documentId, const IR::VectorView<id_t> tokens) {
	for(const id_t & token: tokens) 
		documentTokens_->insert(documentId, token), tokenDocuments_->insert(token, documentId);
	LOG_DEBUG() << "Inserted document id: " << documentId << "; tokens amount: " << tokens.size();
}
...
IR::Vector<id_t> DataBase::findTokensByDocumentId(id_t documentId) {
	...
	LOG_DEBUG() << "For document id: " << documentId << " found " << result.size() << " tokens";
	return result;
}
...
IR::Vector<id_t> DataBase::findDocumentsByTokenId(id_t tokenId) {
	...
	LOG_DEBUG() << "For token id: " << tokenId << " found " << result.size() << " documents";
	return result;
}
} // namespace dbService

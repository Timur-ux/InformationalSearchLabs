...
std::string TokenHandler::HandleRequest(HttpRequest &request,
	...
	for(const auto & id : ids) {
		...
		IR::Vector<std::uint32_t> values = DataBase::instance().findTokensByDocumentId(key);
		responseBody.emplace_back(token::TokenResponseBodyA{key, std::vector<std::uint32_t>{std::begin(values), std::end(values)}});
